...
def test_get_reverse_domains(capsys: pytest.CaptureFixture, data: dict[str, list[str]] | None) -> None:
...
def test_verify_platform(mocker: MockerFixture, name: str, version_offset: int) -> None:
    mocker.patch("service.cli.platform.system", return_value=name)
    mocker.patch(
        "service.cli.platform.mac_ver",
        return_value=[str(MACOS_MIN_VERSION + version_offset)],
    )
    ...
    if name != "Darwin":
        ...
        context = pytest.raises(
            ...
            match=rf".* requires macOS {MACOS_MIN_VERSION} or higher",
        )
...
def test_cli_version(config: Path) -> None:
    runner = CliRunner()
...
def test_cli_verbosity(caplog: pytest.LogCaptureFixture, config: Path, verbose: bool, short_opts: bool) -> None:
    CliRunner().invoke(
        cli,
        [
            "-c",
            ...
            "start",
            "--help",
            ("-v" if short_opts else "--verbose") if verbose else "",
        ],
    )
    assert ("DEBUG" in caplog.text) is verbose


def test_cli_verifies_platform(mocker: MockerFixture, config: Path) -> None:
...
def test_cli_disable(mocker: MockerFixture, config: Path, plist: Path, should_fail: bool) -> None:
    mocker.patch("service.cli.os.getenv", return_value="x")  # use system domain
    mock_run = mocker.patch(
        "service.cli.launchctl.subprocess.run",
...
@pytest.mark.parametrize("should_fail", [True, False])
def test_cli_enable(mocker: MockerFixture, config: Path, plist: Path, should_fail: bool) -> None:
    mocker.patch("service.cli.os.getenv", return_value="x")  # use system domain
    mock_run = mocker.patch(
        "service.cli.launchctl.subprocess.run",
        return_value=subprocess.CompletedProcess([], 0),
...
@pytest.mark.parametrize("should_fail", [True, False])
def test_cli_restart(mocker: MockerFixture, config: Path, plist: Path, should_fail: bool) -> None:
    mocker.patch("service.cli.os.getenv", return_value="x")  # use system domain
    mock_run = mocker.patch(
        "service.cli.launchctl.subprocess.run",
        return_value=subprocess.CompletedProcess([], 0),
